...
pub const Runner = struct {
    ...
    pub fn apply(self: Runner, connection: *Connection) !u32 {
        ...
        for (self.migrations) |migration| {
            ...
            var result = try connection.exec(migration.upSql);
            result.deinit();
...
test "migration runner records applied versions and rolls back once" {
    ...
    const migrations = [_]Migration{
        .{ .version = 1, .upSql = "CREATE TABLE migrated_items (id INTEGER);", .downSql = "DROP TABLE migrated_items;" },
    };
