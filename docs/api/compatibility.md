...
| 2 | WHERE / predicates (comparison, IN, BETWEEN, EXISTS, LIKE/GLOB/REGEXP/MATCH, COLLATE) | `connection/connection.zig` (`matchesContext`, `evalContext`, `havingCompare`, `evalOwnsResult`), `connection/pattern.zig`, `connection/compare.zig` (`nullSafeEqual`), `vm/value.zig` | `pattern.zig`/`compare.zig` unit tests; `connection.zig` predicate + probe tests; `examples/43,44,45,53` | Covered | `TODO(where)`: operand `COLLATE` in `LIKE` patterns still binary (`LIKE`+`COLLATE BINARY` sensitivity); `RTRIM` and outer `(a = b) COLLATE` wrappers unevaluated |
...
| 4 | Scalar / math / datetime / JSON / aggregate functions | `sql/functions/scalar.zig` (`soundex`, `like`/`glob` forms), `math.zig`, `datetime.zig`, `json.zig` (`json_array_length`), `aggregate.zig`, `window.zig` | source-local function tests; `connection.zig` scalar-gap test; `examples/20,36` | Partial | `TODO(func)`: JSON path and datetime corners; `LIKE`+`COLLATE BINARY` sensitivity |
