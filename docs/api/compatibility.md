...
| 4 | Scalar / math / datetime / JSON / aggregate functions | `sql/functions/scalar.zig` (`soundex`, `like`/`glob` forms), `math.zig`, `datetime.zig`, `json.zig` (`json_array_length`), `aggregate.zig`, `window.zig` | source-local function tests; `connection.zig` scalar-gap test; `examples/20,36` | Partial | `TODO(func)`: JSON path and datetime corners |
