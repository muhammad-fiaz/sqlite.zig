...
|---|-------------|-------------|----------------|--------|----------------|
| 1 | SELECT (projections, DISTINCT, ORDER BY, LIMIT/OFFSET) | `connection/connection.zig` (`select`, `selectJoin`, `selectGrouped`), `plan/planner.zig`, `dsl/query_builder.zig` | source-local tests in `connection.zig`; `examples/04,12,14,16,36,54,63,69` | Covered | none: `DISTINCT`+`ORDER BY`, per-arm/outer compound `LIMIT`, FROM-less `ORDER BY` validation verified |
| 2 | WHERE / predicates (comparison, IN, BETWEEN, EXISTS, LIKE/GLOB/REGEXP/MATCH, COLLATE) | `connection/connection.zig` (`matchesContext`, `evalContext`, `havingCompare`, `evalOwnsResult`), `connection/pattern.zig`, `connection/compare.zig` (`nullSafeEqual`), `vm/value.zig` | `pattern.zig`/`compare.zig` unit tests; `connection.zig` predicate + probe tests; `examples/43,44,45,53` | Covered | Explicit `COLLATE` propagates through comparisons, `IN`/`BETWEEN` bounds, the `IS` family, bare expressions, and `ORDER BY` keys (either `COLLATE`-then-direction order); `RTRIM` and outer `(a = b) COLLATE` wrappers are honored (`LIKE` ignores explicit `COLLATE` per the reference) |
| 3 | Expressions (arithmetic, concat, CASE, CAST, parameters) | `sql/expr.zig`, `sql/ast.zig` (`cloneBorrowedExpr`), `connection/connection.zig` (`evalBinary`, `materialize`) | source-local `sql/expr.zig` tests; `connection.zig` cast/overflow probes; `examples/53,58` | Covered | none: overflow widening, text/numeric/BLOB `CAST` edges, expression `BETWEEN` verified |
| 4 | Scalar / math / datetime / JSON / aggregate functions | `sql/functions/scalar.zig` (`soundex`, `like`/`glob` forms), `math.zig`, `datetime.zig`, `json.zig` (`json_array_length`), `aggregate.zig`, `window.zig` | source-local function tests; `connection.zig` scalar-gap test; `examples/20,36` | Partial | `TODO(func)`: older JSON entry points keep text-only inputs (new ones coerce); `json_mergepatch`, `json_array_insert`, `json_error_position` unevaluated |
...
| 8 | Window functions (ROW_NUMBER, RANK, LAG/LEAD, PARTITION BY) | `sql/functions/window.zig`, `sql/parser.zig`, `dsl/column.zig`, `dsl/ast_builder.zig` | `window.zig` unit tests; `parser.zig` named-window tests; `connection.zig` frame probe tests; `examples/64` | Covered | none: `EXCLUDE CURRENT ROW`/`GROUP`/`TIES`, value-scan `RANGE`, and `GROUPS` frames verified |
