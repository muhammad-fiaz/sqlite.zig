...
| 2 | WHERE / predicates (comparison, IN, BETWEEN, EXISTS, LIKE/GLOB/REGEXP/MATCH, COLLATE) | `connection/connection.zig` (`matchesContext`, `evalContext`, `havingCompare`, `evalOwnsResult`), `connection/pattern.zig`, `connection/compare.zig` (`nullSafeEqual`), `vm/value.zig` | `pattern.zig`/`compare.zig` unit tests; `connection.zig` predicate + probe tests; `examples/43,44,45,53` | Covered | `TODO(where)`: `RTRIM` and outer `(a = b) COLLATE` wrappers unevaluated (`LIKE` ignores explicit `COLLATE` per the reference) |
