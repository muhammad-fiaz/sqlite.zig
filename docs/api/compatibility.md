...
|---|-------------|-------------|----------------|--------|----------------|
| 1 | SELECT (projections, DISTINCT, ORDER BY, LIMIT/OFFSET) | `connection/connection.zig` (`select`, `selectJoin`, `selectGrouped`), `plan/planner.zig`, `dsl/query_builder.zig` | source-local tests in `connection.zig`; `examples/04,12,14,16,36,54,63,69` | Covered | `TODO(select)`: `ORDER BY` validation on FROM-less selects; `DISTINCT`+`ORDER BY`, per-arm and outer compound `LIMIT` verified |
| 2 | WHERE / predicates (comparison, IN, BETWEEN, EXISTS, LIKE/GLOB/REGEXP/MATCH, COLLATE) | `connection/connection.zig` (`matchesContext`, `evalContext`, `havingCompare`), `connection/pattern.zig`, `connection/compare.zig` (`nullSafeEqual`), `vm/value.zig` | `pattern.zig`/`compare.zig` unit tests; `connection.zig` predicate + probe tests; `examples/43,44,45,53` | Covered | `TODO(where)`: operand `COLLATE` in `IN` lists / `LIKE` patterns / `BETWEEN` bounds still binary; `RTRIM` and outer `(a = b) COLLATE` wrappers unevaluated |
| 3 | Expressions (arithmetic, concat, CASE, CAST, parameters) | `sql/expr.zig`, `sql/ast.zig`, `connection/connection.zig` (`evalBinary`, `materialize`) | source-local `sql/expr.zig` tests; `connection.zig` cast/overflow probes; `examples/53,58` | Covered | `TODO(expr)`: BLOB `CAST` rendering parity unprobed; overflow widening and text/numeric `CAST` edges verified |
