...
pub fn main() !void {
    ...
    defer check.deinit();
    if (check.count() != 1 or !std.mem.eql(u8, check.rows[0][0].text, "ok")) return error.FormatMismatch;
    db.close();
    db = try sqlite.open(std.heap.page_allocator, "example_07.db");
    var result = try db.exec("SELECT name FROM users WHERE id = 7;");
    defer result.deinit();
    if (result.count() != 1 or !std.mem.eql(u8, result.rows[0][0].text, "Compat")) return error.FormatMismatch;
