...
pub fn main() !void {
    ...
    defer db.close();
    try db.createTable(User, .{ .if_not_exists = true, .primary_key_key = User.key("id"), .unique_keys = &.{User.key("email")} });
    try db.createTable(Order, .{ .if_not_exists = true, .primary_key_key = Order.key("id"), .foreign_keys = &.{.{ .table = "relation_users", .referenced_column = "id", .column_key = Order.key("user_id") }} });
