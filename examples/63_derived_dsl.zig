...
pub fn main() !void {
    ...
    var big = try db.from("dd_orders").select(.{db.col("user_id")}).where(db.col("amount").gt(100)).asSubquery("big");
    ...
    for (rawBig.rows, 0..) |row, i| {
        if (dynBig.rows[i][0].integer != row[0].integer) return error.VerificationFailed;
    }
    var typedSub = try db.from(Order).select(.{Order.columns.user_id}).where(Order.columns.amount.gt(100)).asSubquery("o");
    var typedOut = try typedSub.select(.{db.col("user_id")}).orderBy(db.col("user_id").asc()).fetch();
    defer typedOut.deinit();
    if (typedOut.count() != 3) return error.VerificationFailed;
    if (typedOut.rows[0][0].integer != 7) return error.VerificationFailed;
    var dynJoin = try big.select(.{db.col("name")}).innerJoin("dd_users", db.col("big.user_id").eq(db.col("dd_users.id"))).orderBy(db.col("name").asc()).fetch();
    defer dynJoin.deinit();
    if (dynJoin.count() != 3) return error.VerificationFailed;
    if (!std.mem.eql(u8, dynJoin.rows[0][0].text, "eight")) return error.VerificationFailed;
    var dynCount = try big.countStar().fetch();
    defer dynCount.deinit();
    if (dynCount.rows[0][0].integer != 3) return error.VerificationFailed;
    var nested = try db.from("dd_orders").select(.{db.col("id")}).asSubquery("a");
    if (nested.asSubquery("b")) |_| {
        return error.VerificationFailed;
    } else |err| {
        if (err != error.InvalidSql) return error.VerificationFailed;
    }
    if (db.from("dd_orders").asSubquery("")) |_| {
        return error.VerificationFailed;
    } else |err| {
        if (err != error.InvalidSql) return error.VerificationFailed;
    }
    ...
    if (afterInsert.rows[0][0].integer != 4) return error.VerificationFailed;
    var dynAfter = try big.select(.{db.col("user_id")}).fetch();
    ...
    var persisted = try reopened.from("dd_orders").select(.{reopened.col("user_id")}).where(reopened.col("amount").gt(100)).asSubquery("big");
    var persistedOut = try persisted.select(.{reopened.col("user_id")}).orderBy(reopened.col("user_id").asc()).fetch();
    defer persistedOut.deinit();
    if (persistedOut.count() != 4) return error.VerificationFailed;
    if (persistedOut.rows[0][0].integer != 7) return error.VerificationFailed;
