...
pub fn main() !void {
    ...
    var trigger = try db.exec("CREATE TRIGGER events_after_insert AFTER INSERT ON trigger_events BEGIN INSERT INTO trigger_audit (id, message) VALUES (1, 'inserted'); END;");
    trigger.deinit();
    ...
    defer audit.deinit();
    if (audit.rowCount() != 1) return error.TriggerVerificationFailed;
    std.debug.print("23 triggers: raw trigger DDL and typed DSL mutation verified\n", .{});
}
