...
pub fn main() !void {
    ...
    if (rawSeeDyn.count() != 1 or !std.mem.eql(u8, rawSeeDyn.rows[0][0].text, "Zed")) return error.VerificationFailed;
    ...
    if (rawSeeTyped.count() != 1 or rawSeeTyped.rows[0][0].integer != 19) return error.VerificationFailed;
