...
pub fn main() !void {
    ...
    if (dynamic.count() != 2 or dynamic.rows[1][0].integer != 3) return error.DynamicCteVerificationFailed;
    ...
    defer typed.deinit();
    if (typed.count() != 2 or !std.mem.eql(u8, typed.rows[0].name, "Ada")) return error.TypedCteVerificationFailed;
