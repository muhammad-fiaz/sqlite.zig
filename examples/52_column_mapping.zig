...
pub fn main() !void {
    ...
    defer typed.deinit();
    if (typed.count() != 1 or !std.mem.eql(u8, typed.rows[0].firstName, "Grace")) return error.TypedMappingVerificationFailed;
    ...
    defer raw.deinit();
    if (raw.count() != 2 or !std.mem.eql(u8, raw.rows[0][0].text, "Ada")) return error.RawMappingVerificationFailed;
