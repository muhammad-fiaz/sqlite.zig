...
pub fn main() !void {
    ...
    if (!std.mem.eql(u8, rawSeen.rows[0][0].text, "Ann")) return error.VerificationFailed;
    if (rawSeen.rows[0][1].integer != 5) return error.VerificationFailed;

    var typedUpBase = try db.from(Account).onConflict(Account.columns.email).doUpdate(.{ .name = db.excluded("name"), .stock = db.excluded("stock") });
    ...
    if (checkTyped.rows[0][0].integer != 1) return error.VerificationFailed;
    if (!std.mem.eql(u8, checkTyped.rows[0][1].text, "Annie")) return error.VerificationFailed;
    if (checkTyped.rows[0][2].integer != 8) return error.VerificationFailed;

    var dynBase = try db.from("upsert_accounts").onConflict(db.col("email")).doUpdate(.{ .stock = 42 });
    ...
    if (!std.mem.eql(u8, checkDyn.rows[0][0].text, "Annie")) return error.VerificationFailed;
    if (checkDyn.rows[0][1].integer != 42) return error.VerificationFailed;

    var skipped = try db.from(Account).onConflict(Account.columns.email).doNothing().insert(.{ .id = 4, .email = "a@x.test", .name = "Nope", .stock = 0 });
    skipped.deinit();
    var checkSkipped = try db.exec("SELECT count(*) FROM upsert_accounts;");
    defer checkSkipped.deinit();
    if (checkSkipped.rows[0][0].integer != 1) return error.VerificationFailed;

    var guardedBase = try db.from(Account).onConflict(Account.columns.email).doUpdate(.{ .stock = 99 });
    ...
    if (checkGuarded.rows[0][0].integer != 42) return error.VerificationFailed;

    var retBase = try db.from(Account).onConflict(Account.columns.email).doUpdate(.{ .stock = db.excluded("stock") });
    var retWithCols = retBase.returning(.{Account.columns.stock});
    var retUp = try retWithCols.insert(.{ .id = 6, .email = "a@x.test", .name = "Annie", .stock = 55 });
    defer retUp.deinit();
    if (retUp.count() != 1) return error.VerificationFailed;
    if (retUp.rows[0][0].integer != 55) return error.VerificationFailed;

    if (db.from(Account).onConflict(Account.columns.email).insert(.{ .id = 7, .email = "a@x.test", .name = "No", .stock = 0 })) |r| {
        var owned = r;
        owned.deinit();
    ...
    if (afterInvalid.rows[0][0].integer != 1) return error.VerificationFailed;

    {
        var check = try db.exec("SELECT email, stock FROM upsert_accounts;");
        defer check.deinit();
        if (check.rows[0][1].integer != 55) return error.VerificationFailed;
    }
    db.close();
    ...
    if (!std.mem.eql(u8, persisted.rows[0][0].text, "a@x.test")) return error.VerificationFailed;
    if (persisted.rows[0][1].integer != 55) return error.VerificationFailed;
