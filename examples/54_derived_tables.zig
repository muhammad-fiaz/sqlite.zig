...
pub fn main() !void {
    ...
    if (aliased.at(0)[0].integer != 1) return error.VerificationFailed;
    if (!std.mem.eql(u8, aliased.at(1)[1].text, "Cy")) return error.VerificationFailed;

    var bare = try db.exec("SELECT count(*) FROM (SELECT id FROM orders);");
    defer bare.deinit();
    if (bare.at(0)[0].integer != 4) return error.VerificationFailed;
    ...
    if (!std.mem.eql(u8, totals.at(0)[0].text, "Ada")) return error.VerificationFailed;
    if (totals.at(0)[1].integer != 200) return error.VerificationFailed;
    if (!std.mem.eql(u8, totals.at(1)[0].text, "Bob")) return error.VerificationFailed;
    ...
    if (nested.at(0)[0].integer != 10) return error.VerificationFailed;
    ...
    if (checkCopy.at(0)[0].integer != 2) return error.VerificationFailed;
    if (checkCopy.at(0)[1].integer != 320) return error.VerificationFailed;
    ...
    if (shadowed.at(0)[0].integer != 99) return error.VerificationFailed;
